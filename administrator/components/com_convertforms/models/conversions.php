...
class ConvertFormsModelConversions extends JModelList
{
    ...
    protected function populateState($ordering = 'a.id', $direction = 'desc')
    {
        ...
        $columns = is_array($columns) ? array_filter($columns) : $columns;
    ...
    protected function getListQuery()
    {
        ...
        if ($filter != '' && $filter != 'skip')
        {
            ...
            {
                $query->where($db->quoteName('a.state') . ' = ' . $filter);
            } else 
            {
                $query->where('(' . $db->quoteName('a.state') . ' IN (' . $filter . '))');
            }
        }
    ...
    public function getItems()
    {
        ...
        foreach ($items as $key => $item)
        {
            ...
            $item->created = ConvertForms\Helper::formatDate($item->created);
