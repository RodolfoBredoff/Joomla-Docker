...
class SunFwAjaxAdmin extends SunFwAjax
{
	...
	public function getAction()
	{
		...
		$this->dbo->setQuery(
			'SELECT id FROM #__template_styles WHERE template = ' . $this->dbo->quote($this->templateName)
			. 'ORDER BY home DESC, id ASC',
			0,
			1
		);
		...
		$this->dbo->setQuery(
			'SELECT COUNT(id) FROM #__sunfw_styles WHERE template = ' . $this->dbo->quote($this->templateName)
		);
		...
		if (
			...
			&&
			method_exists('JLanguageHelper', 'getInstalledLanguages')
		)
		{
		...
		$ajaxBase = "{$root}administrator/index.php?option=com_ajax&format=json&group=system&plugin=sunfw"
			. "&template_name={$this->templateName}&style_id={$this->styleID}&" . JSession::getFormToken() . '=1';
		...
		// Get Getting Started Series link.
		$this->gettingStartedSeriesLink = SUNFW_GETTING_STARTED_SERIES_URL;
		...
		{
			$info = SunFwHttp::get(SUNFW_GET_INFO_URL . "&token={$params['token']}&identified_name=" . SUNFW_ID, 3600);
		}
		...
		$this->setResponse(array(
			'bootstrapVersion' => SunFwSiteBootstrap::getVersion($this->templateName),
			'liveChatPopup' => isset($pluginParams['live_chat_popup']) ? $pluginParams['live_chat_popup'] : null,
			'maxLoading' => isset($pluginParams['max_loading']) ? (int) $pluginParams['max_loading'] : 0,
			...
			'ga' => array(
				...
				'profile' => (isset($info) && is_array($info['message'])) ? $info['message']['ga_property_id'] : 0,
				'set' => array(
			...
			'urls' => array(
				...
				'gettingStartedSeriesLink' => $this->gettingStartedSeriesLink,
				'getLiveChatHtml' => '&action=getLiveChat',
				'savePluginParams' => '&action=savePluginParams',
				'CgZp5GTv' => '&action=purchase',
				'Mrk1ZsC2' => '&action=getBanner',
				'GcPAeQCn' => '&action=getDiscount',
				'yuqT1eqf' => '&context=admin&action=tryPro',
				'nN8dY5cp' => '&context=sample-data&action=get',
				'K1m0cfHG' => SUNFW_CUSTOMER_AREA,
				'chosen' => array(
					'css' => ( JFile::exists(JPATH_ROOT . '/media/legacy/css/chosen.min.css')
						? '/media/legacy/css/chosen.min.css'
						: '/media/jui/css/chosen.css' ),
					'js' => ( JFile::exists(JPATH_ROOT . '/media/legacy/js/chosen.min.js')
			...
			'textMapping' => array(
				...
				'see-tutorial' => JText::_('SUNFW_SEE_TUTORIAL'),
				...
				'only-alphanumeric-characters-are-supported' => JText::_('SUNFW_ONLY_ALPHANUMERIC_CHARACTERS_ARE_SUPPORTED'),
				...
				'K53G78yE-xtJ1Gs8h' => JText::_('SUNFW_TIME_TO_GO_PRO_NO_THANKS_BUTTON'),
				...
				'xpz0tWTT' => JText::_('SUNFW_DISCOUNT_OFFER_EXPIRED'),
				'UccWXSfV' => empty($params['clickedPurchase']) ? null : $params['clickedPurchase'],
				'qUTvD2zQ' => empty($params['currentDiscount']) ? null : $params['currentDiscount'],

			    'read-getting-started-series' => JText::_('SUNFW_READ_GETTING_STARTED_SERIES'),
			    'read-getting-started-series-intro' => JText::_('SUNFW_READ_GETTING_STARTED_SERIES_INFO'),
			    'read-guide' => JText::_('SUNFW_READ_GUIDE'),
			    'video-guide' => JText::_('SUNFW_VIDEO_GUIDE'),
			    'getting-started' => JText::_('SUNFW_GETTING_STARTED'),
			    'get-help' => JText::_('SUNFW_GET_HELP')
			)
		));
	}
	...
	protected function getComponentAlerts()
	{
		...
		$items = array_merge(array(), $this->app->triggerEvent('SunFwGetComponentAlerts'));
	...
	protected function getComponentHeader()
	{
		...
		$data = array(
			'logo' => array(
				...
				'title' => htmlspecialchars(
					$this->app->getCfg('sitename'), ENT_QUOTES, 'UTF-8'
				)
			),
			'menu' => array(
				array(
				...
				array(
					...
					'items' => array(
						...
						array(
							'href' => '#',
							'type' => 'trigger-other',
							'title' => JText::_('SUNFW_LIVE_CHAT_SETTINGS_TAB'),
							'target' => '#live-chat-settings'
						)
					)
				),
				array(
			...
			'textMapping' => array(
				'y37Rm2cT' => JText::sprintf('SUNFW_UPGRADE_TO_PRO_FOR_ADVANCED_FEATURES', $this->templateName),
				...
				'cQVnkpgC' => JText::_('SUNFW_RENEW_LITE'),
		...
		$data = array_merge_recursive($data, $this->app->triggerEvent('SunFwGetComponentHeader'));
	...
	protected function getComponentBody()
	{
		...
		$saveAsCopy = "index.php?option=com_ajax&format=json&group=system&plugin=sunfw&context=admin"
			. "&action=saveAsCopy&template_name={$this->templateName}&style_id={$this->styleID}&"
		...
		{
			$inputs[substr(basename($input), 0, -3)] = $root . str_replace(
				JPATH_ROOT, '', $input
			);
		}
		...
		$data = array(
			'tabs' => array(
				array(
					...
					'render' => array(
						...
						'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/layout"
							. "/render.js?{$vd}"
					),
				...
				array(
					...
					'render' => array(
						...
						'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/styles"
							. "/render.js?{$vd}"
					),
					'className' => 'toggle-pane'
				),
				array(
					...
					'render' => array(
						...
						'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/mega-menu"
							. "/render.js?{$vd}"
					),
					'className' => 'toggle-pane'
				),
				array(
					...
					'render' => array(
						...
						'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/system"
							. "/render.js?{$vd}"
					),
					'className' => 'toggle-pane'
				),
					...
					'items' => array(
						array(
							...
							'render' => array(
								...
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/cookie-law"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						),
						array(
							...
							'render' => array(
								...
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/social-share"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						),
						array(
							...
							'render' => array(
								...
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/commenting"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						),
						array(
							...
							'render' => array(
								'name' => 'PaneCustom404',
								'data' => ( new SunFwAjaxCustom404() )->getAction(true),
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/custom-404"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						),
						array(
							'href' => '#coming-soon',
							'title' => JText::_('SUNFW_COMING_SOON_TAB'),
							'render' => array(
								'name' => 'PaneComingSoon',
								'data' => ( new SunFwAjaxComingSoon() )->getAction(true),
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/coming-soon"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						)
				...
				array(
					...
					'render' => array(
						...
						'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/assignment"
							. "/render.js?{$vd}"
					),
					'className' => 'toggle-pane'
				),
				array(
					...
					'items' => array(
						array(
							...
							'render' => array(
								'name' => 'PaneSampleData',
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/sample-data"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						),
						array(
							...
							'render' => array(
								...
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/maintenance"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						)
					...
					'items' => array(
						array(
							...
							'render' => array(
								...
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/user-account"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						),
						array(
							...
							'render' => array(
								...
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/privacy-settings"
									. "/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						),
						array(
							'href' => '#live-chat-settings',
							'title' => JText::_('SUNFW_LIVE_CHAT_SETTINGS_TAB'),
							'render' => array(
								'name' => 'PaneLiveChatSettings',
								'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js"
									. "/live-chat-settings/render.js?{$vd}"
							),
							'className' => 'toggle-pane'
						)
					)
				...
				array(
					...
					'render' => array(
						...
						'file' => "{$root}/plugins/system/sunfw/assets/joomlashine/admin/js/about"
							. "/render.js?{$vd}"
					),
					'listClass' => 'hidden',
					'className' => 'toggle-pane'
			...
			'textMapping' => array(
				...
				'pane-layout-Gjfd4Sxv' => JText::_('SUNFW_PANE_LAYOUT_LIMITATION_TITLE'),
				'pane-layout-td6Tz2Gb' => JText::_('SUNFW_PANE_LAYOUT_LIMITATION_MESSAGE'),

				'pane-layout-custom-Gjfd4Sxv' => JText::_('SUNFW_PANE_CUSTOM_LAYOUT_LIMITATION_TITLE'),
				'pane-layout-custom-td6Tz2Gb' => JText::_('SUNFW_PANE_CUSTOM_LAYOUT_LIMITATION_MESSAGE'),
				...
				'pane-coming-soon-Gjfd4Sxv' => JText::_('SUNFW_PANE_COMING_SOON_LIMITATION_TITLE'),
				'pane-coming-soon-td6Tz2Gb' => JText::_('SUNFW_PANE_COMING_SOON_LIMITATION_MESSAGE'),
				...
				'icon-selector' => JText::_('SUNFW_ICON_SELECTOR'),
				'all-icons' => JText::_('SUNFW_ICON_SELECTOR_ALL_ICONS'),
				'new-icons-4-7' => JText::_('SUNFW_ICON_SELECTOR_NEW_47'),
				'web-development' => JText::_('SUNFW_ICON_SELECTOR_WEB_DEVELOPMENT'),
				'accessibility' => JText::_('SUNFW_ICON_SELECTOR_ACCESSIBILITY'),
				'hand' => JText::_('SUNFW_ICON_SELECTOR_HAND'),
				'transportation' => JText::_('SUNFW_ICON_SELECTOR_TRANSPORTATION'),
				'gender' => JText::_('SUNFW_ICON_SELECTOR_GENDER'),
				'file-type' => JText::_('SUNFW_ICON_SELECTOR_FILE_TYPE'),
				'spinner' => JText::_('SUNFW_ICON_SELECTOR_SPINNER'),
				'form-control' => JText::_('SUNFW_ICON_SELECTOR_FORM_CONTROL'),
				'payment' => JText::_('SUNFW_ICON_SELECTOR_PAYMENT'),
				'chart' => JText::_('SUNFW_ICON_SELECTOR_CHART'),
				'currency' => JText::_('SUNFW_ICON_SELECTOR_CURRENCY'),
				'text-editor' => JText::_('SUNFW_ICON_SELECTOR_TEXT_EDITOR'),
				'directional' => JText::_('SUNFW_ICON_SELECTOR_DIRECTIONAL'),
				'video-player' => JText::_('SUNFW_ICON_SELECTOR_VIDEO_PLAYER'),
				'brand' => JText::_('SUNFW_ICON_SELECTOR_BRAND'),
				'medical' => JText::_('SUNFW_ICON_SELECTOR_MEDICAL'),

				'settings-live-chat-popup' => JText::_('SUNFW_LIVE_CHAT_SETTINGS_ENABLE_POPUP'),
				'settings-live-chat-popup-hint' => JText::_('SUNFW_LIVE_CHAT_SETTINGS_ENABLE_POPUP_HINT')
			)
		);
		...
		if (
			!$xml
			||
			!($xml = current($xml->xpath('//feature[@name="custom_404"]')))
			||
			(string) $xml['enabled'] !== 'yes'
		)
		{
		...
		// Check if the Custom Coming Soon feature is supported by the current template.
		$xml = SunFwHelper::getManifest($this->templateName);

		if (
			!$xml
			||
			!($xml = current($xml->xpath('//feature[@name="coming_soon"]')))
			||
			(string) $xml['enabled'] !== 'yes'
		)
		{
		...
		$data = array_merge_recursive($data, $this->app->triggerEvent('SunFwGetComponentBody'));
	...
	protected function getComponentFooter()
	{
		...
		else
		{
			$otherFooter = array(
				sprintf(
					...
					'<a href="http://www.joomlashine.com/joomla-extensions/jsn-pagebuilder.html" target="_blank"'
					. ' rel="noopener noreferrer" class="link-normal sunfw-prd">JSN PageBuilder</a>'
				)
			);
		}
		...
		// Allow 3rd-party plugins to add their own tabs into main workspace.
		$data = array_merge_recursive($data, $this->app->triggerEvent('SunFwGetComponentFooter'));
	...
	{
		$ajaxServer = "index.php?option=com_ajax&format=json&group=system&plugin=sunfw&context=update&"
			. "style_id={$this->styleID}&template_name={$this->templateName}&"
			. JSession::getFormToken() . '=1&';
	...
	protected function getLicense()
	{
		...
		if (JFile::exists("{$tmp_path}/{$this->template['id']}/license.data"))
		{
			JFile::delete("{$tmp_path}/{$this->template['id']}/license.data");
		}

		// Get license data.
		$license = SUNFW_GET_LICENSE_URL . "&identified_name={$this->template['id']}";
		$license .= '&domain=' . JUri::getInstance()->toString(array('host'));
		$license .= "&ip={$_SERVER['SERVER_ADDR']}&token={$params['token']}";
		$license = SunFwHttp::get(
			$license,
			86400,
			"{$tmp_path}/{$this->template['id']}/license"
		);
		...
		elseif (!$license || $license['result'] == 'failure')
		{
			...
			error_log($license ? $msg : json_last_error_msg());
		}
	...
	public function loadJsonFileAction()
	{
		...
		if (!empty($root) && strpos($file, $root) === 0)
		{
	...
	public function tryProAction()
	{
		...
		$result = SUNFW_JOIN_TRIAL_URL . "&identified_name={$this->template['id']}";
		$result .= '&domain=' . JUri::getInstance()->toString(array('host'));
		$result .= "&ip={$_SERVER['SERVER_ADDR']}&token={$params['token']}";
		$result = SunFwHttp::get($result);
