...
class SunFwAjaxSampleData extends SunFwAjax
{
	...
	public function getAction($return = false)
	{
		...
		$packages = array_merge($packages, $this->app->triggerEvent('SunFwGetSampleDataPackages'));
	...
	public function importPackageAction()
	{
		try
		{
			...
			if (preg_match('/[0-9]\.[0-9]+\.[0-9]+/', $databaseVersion, $dbMatch))
			{
			...
			foreach ($this->xml->xpath('//extension[@author="3rd_party"]') as $component)
			{
				if (isset($component['author']) && $component['author'] == '3rd_party')
				{
					...
					if (isset($namePrefix[$extensionType]) && strpos((string) $attrs['name'], $namePrefix[$extensionType]) === false)
					{
						$componentName = $namePrefix[$extensionType] . (string) $attrs['name'];
					}
					else
					{
						$componentName = (string) $attrs['name'];
					}
			...
			$users = $this->dbo->setQuery('SELECT * FROM #__users;')->loadAssocList();
			...
			foreach ($this->xml->xpath('//extension') as $extension)
			{
				...
				if (isset($namePrefix[$extensionType]) && strpos((string) $extension['name'], $namePrefix[$extensionType]) === false)
				{
					$extensionName = $namePrefix[$extensionType] . (string) $extension['name'];
				}
				else
				{
				...
				elseif (!empty($extension['type']))
				{
					// Check if 3rd-party component is installed.
					$canInstall = SunFwHelper::isExtensionInstalled($extensionName, (string) $extension['type']);
					...
					if (!$canInstall)
					{
						...
						$tmpAttentions = array(
							...
							'display' => ( $extension['type'] !== 'component' || count($queries) ) ? true : false,
				...
				if ($canInstall)
				{
					...
					{
						// If the users table is to be dropped, set a flag to restore users later.
						if (stripos($query, 'DROP TABLE ') !== false && stripos($query, '_users') !== false)
						{
							$restoreUsers = true;
						}
							...
							foreach ($matches as $match)
							{
								...
								if (isset($this->mediaFolder))
								{
									...
									if (!is_file($mediaFile))
									{
										try
										{
											$data = SunFwHttp::get($match[0]);

											// Write downloaded data to local file.
											if (!JFile::write($mediaFile, $data))
											{
					...
					// Copy embedded files if has.
					foreach ($extension->xpath('files/file') as $file)
					{
						if (JFile::exists($this->temporary_path . '/' . $file))
						{
							JFile::move($this->temporary_path . '/' . $file, JPATH_ROOT . '/' . $file);
						}
					}
					...
					if (isset($extension['ext-style-package']))
					{
						...
						$fileUrl = 'https://www.joomlashine.com/index.php?option=com_lightcart&controller=remoteconnectauthentication&task=authenticate&tmpl=component&upgrade=yes&identified_name=ext_style&edition=&joomla_version='
							. SunFwHelper::getJoomlaVersion(2) . '&file_attr={"identified_template_name":"' . $this->template->id . '","ext_style":"' . (string) $extension['ext-style-package'] . '"}';

						// Download file to temporary folder.
						$data = SunFwHttp::get($fileUrl);
						$path = $config->get('tmp_path') . "/{$this->template->id}/" . (string) $extension['ext-style-package'] . '_ext_style.zip';
						...
						// Remove downloaded extended style package.
						JFile::delete($path);
					}
					...
					elseif ('com_osproperty' == $extensionName)
					{
					...
					// Manipulate data for TZ Portfolio Plus.
					elseif ('com_tz_portfolio_plus' == $extensionName)
					{
						// Set the current logged in user as article creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								...
								->where('created_by != ' . $user->id, 'OR')
								->where('modified_by != ' . $user->id, 'OR')
								->where('checked_out != 0', 'OR')
						)->execute();

						// Set the current logged in user as category creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__tz_portfolio_plus_categories')
								->set('created_user_id = ' . $user->id)
								->set('modified_user_id = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
								->where('created_user_id != ' . $user->id, 'OR')
								->where('modified_user_id != ' . $user->id, 'OR')
								->where('checked_out != 0', 'OR')
						)->execute();

						// Set the current logged in user as field creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__tz_portfolio_plus_fields')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
					...
					// Manipulate data for Event Booking.
					elseif ('com_eventbooking' == $extensionName)
					{
						// Set the current logged in user as event creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__eb_events')
								->set('created_by = ' . $user->id)
					...
					// Manipulate data for redSHOP.
					elseif ('com_redshop' == $extensionName)
					{
						// Set the current logged in user as category creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__redshop_category')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
						...
						// Set the current logged in user as coupon creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__redshop_coupons')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
						...
						// Set the current logged in user as currency creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
						...
						// Set the current logged in user as field group creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__redshop_fields_group')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
						...
						// Set the current logged in user as manufacturer creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__redshop_manufacturer')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
						...
						// Set the current logged in user as mass discount creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__redshop_mass_discount')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
						...
						// Set the current logged in user as order status creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__redshop_order_status')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
						...
						// Clear checked out status for products.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
					...
					// Manipulate data for JTicketing.
					elseif ('com_jticketing' == $extensionName)
					{
						// Set the current logged in user as coupon creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__jticketing_coupon')
								->set('created_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
								->where('created_by != ' . $user->id, 'OR')
						...
						// Set the current logged in user as reminder type creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__jticketing_reminder_types')
								->set('created_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
								->where('created_by != ' . $user->id, 'OR')
						...
						// Set the current logged in user as venue creator.
						$this->dbo->setQuery(
							$this->dbo->getQuery(true)
								->update('#__jticketing_venues')
								->set('created_by = ' . $user->id)
								->set('modified_by = ' . $user->id)
								->set('checked_out = 0')
								->set("checked_out_time = '0000-00-00 00:00:00'")
			...
			// Restore the users table if necessary.
			if (isset($restoreUsers) && $restoreUsers)
			{
				foreach ($users as $u)
				{
					$this->dbo->setQuery(
						'REPLACE INTO #__users
						(' . implode(', ', array_map(array($this->dbo, 'quoteName'), array_keys($u))) . ')
						VALUES
						(' . implode(', ', array_map(array($this->dbo, 'quote'), array_values($u))) . ');'
					)->execute();
				}
			}
			...
			// Clear checked out status for module instances.
			$this->dbo->setQuery(
				$this->dbo->getQuery(true)
					->update('#__modules')
					->set('checked_out = 0')
			...
			// Update Joomla's database schemas if necessary.
			$this->dbo->setQuery('SELECT version_id FROM #__schemas WHERE extension_id = 700');

			if (($version = $this->dbo->loadResult())
				&& version_compare(SunFwHelper::getJoomlaVersion(), $version, '>'))
			{
				$installer = JInstaller::getInstance();

				if ($manifest = $installer->isManifest(JPATH_MANIFESTS . '/files/joomla.xml'))
				{
					// Get schema paths.
					$schemapaths = $manifest->update->schemas->children();

					if (count($schemapaths))
					{
						$dbDriver = strtolower($this->dbo->name);
						...
						if (!empty($schemapath)
							&& !empty($files = JFolder::files(JPATH_ROOT . "/{$schemapath}", '\.sql$')))
						{
							$files = str_replace('.sql', '', $files);

							usort($files, 'version_compare');

							// Loop thru schema update files and import if necessary.
							foreach ($files as $file)
							{
								if (version_compare($file, $version, '>')
									&& $buffer = file_get_contents(JPATH_ROOT . "/{$schemapath}/{$file}.sql"))
								{
									...
									foreach ($queries as $query)
									{
										$this->dbo->setQuery($this->dbo->convertUtf8mb4QueryToUtf8($query));
							...
							{
								$query->clear()
									->insert($this->dbo->quoteName('#__schemas'))
									->columns(array($this->dbo->quoteName('extension_id'), $this->dbo->quoteName('version_id')))
									->values('700, ' . $this->dbo->quote(end($files)));
	...
	public function restoreBackupAction()
	{
		...
		if (!$backups || !count($backups))
		{
			$tmpPath = JFactory::getConfig()->get('tmp_path') . "/{$this->template->id}/{$this->sample['id']}/backups/backups";
			$backups = glob("{$tmpPath}/*.sql");
	...
	public function installStructureAction()
	{
			...
			foreach ($matches as $match)
			{
				...
				if (!is_file($mediaFile))
				{
					try
					{
						...
						// Download demo asset.
						$data = SunFwHttp::get("{$link}/{$image}");

						// Write downloaded data to local file.
						if (JFolder::create(dirname($mediaFile)))
						{
							JFile::write($mediaFile, $data);
						}
					}
					catch (Exception $e)
					{
		...
		// Drop any column that is not exists in database table.
		$columns = $this->dbo->getTableColumns('#__sunfw_styles');
		...
		if (count($style))
		{
			try
			{
				...
				$this->dbo->updateObject('#__sunfw_styles', $params, 'style_id');
	...
	protected function extract()
	{
		if (!isset($this->xml))
		{
			...
			if (!empty($joomla_ver))
			{
				if (version_compare(SunFwHelper::getJoomlaVersion(), $joomla_ver, '<'))
				{
	...
	protected function getExtensionState($name, $version, $isThirdParty = false, $type = 'component')
	{
		...
		if ($isThirdParty)
		{
			// If version has more than 3 numbers, just ignore number from 3rd position.
			$current_version = explode('.', $installedExtension->version);
			$desired_version = explode('.', $version);

			if (version_compare("{$current_version[0]}.{$current_version[1]}", "{$desired_version[0]}.{$desired_version[1]}", '>'))
			{
				return 'unsupported';
			}
		}
	...
	protected function backupDatabase()
	{
		...
		{
			if (strpos($table->Name, $this->dbo->getPrefix()) !== 0)
			{
				continue;
			}
		...
		// Prepare path to store the final backup file.
		...
		require_once SUNFW_PATH . '/libraries/3rd-party/php-zip/src/Zip.php';

		try
		{
			$zip = new Zip();

			$zip->zip_start($zip_path);

			// Add all SQL files to backup archive.
