...
if (@count($component['rows']))
{
	...
	if (count($rows))
	{
		...
		else
		{
 		...
 		$section_visible_in = isset($component['settings']['visible_in']) ? $component['settings']['visible_in'] : array();
 		$class .= ' ' . SunFwSiteBootstrap::showOn($section_visible_in);

		$section_items_html = '<div id="sunfw_' . $component['id'] . '" '
			. 'class="sunfw-section sunfw_' . $component['id'] . ' ' . $classSticky
			. trim($class)
			. '"' . $layoutViewer . '>'
			. '<div class="' . ( isset($component['settings']['full_width']) ? 'container-fluid' : 'container' ) . '">';
