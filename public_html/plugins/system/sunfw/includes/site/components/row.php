...
{
	$bootstrapVersion = SunFwSiteBootstrap::getVersion();
	$columns = array();
	...
	foreach ($component['columns'] as $i => $columnIndex)
	{
        ...
        {
	        $column['width']['small-smartphone'] = isset($column['width']['small-smartphone'])
                ? $column['width']['small-smartphone']
                : 12;
        }
	...
	$row_visible_in = isset($component['settings']['visible_in']) ? $component['settings']['visible_in'] : array();
	$class .= ' ' . SunFwSiteBootstrap::showOn($row_visible_in);
	?>
<div id="<?php echo $component['id']; ?>" class="row <?php echo trim($class); ?>">
		<?php
	// Prepare column width.
	...
	if (!$allColumnsVisible)
	{
		...
		foreach ($component['columns'] as $i => $columnIndex)
		{
			...
			{
				// Remaining span will be split equally for columns in the right side.
				// E.g. if the row has 5 column then 2 remaining span will be split equally
		...
		if ($hasMainBody > -1)
		{
			foreach ($component['columns'] as $i => $columnIndex)
			{
				if (!isset($columns[$i]))
				{
					...
					if ($bootstrapVersion === '4')
					{
						$columns[$hasMainBody]['width']['small-smartphone']
                            += $layout['columns'][$columnIndex]['width']['small-smartphone'];
					}
				}
			}
		}
	...
	foreach ($columns as $i => $column)
	{
		...
		if ($allColumnsVisible || $hasMainBody > -1)
		{
			...
			if ($bootstrapVersion === '4')
			{
				$small_smartphone = $column['width']['small-smartphone'];
			}
		}
		...
		else
		{
			...
			if (!isset($columns[$i + 1]) && !isset($columns[$i + 2]) && isset($component['columns'][$i + 1]))
			{
				...
				if ($bootstrapVersion === '4')
				{
					$span['small-smartphone'] += $invisibleColumn['width']['small-smartphone'];
				}
			}
			...
			foreach ($screens as $screen => $equalized)
			{
                ...
                $name = ($screen === 'small-smartphone' ? 'small_smartphone' : $screen);
				${$name} = $column['width'][$screen];
		...
		{
			$class = SunFwSiteBootstrap::colWidth(array(
                'col-small-smartphone' => @$small_smartphone ?: 12,
                'col-xs' => $xs,
                'col-sm' => $sm,
                'col-md' => $md,
                'col-lg' => $lg
		...
		else
		{
			...
			$class = $bootstrapVersion !== '4' ? "col-xs-{$lg}" : "col-{$lg}";
		}
		...
		$visible_in = isset($column['settings']['visible_in']) ? $column['settings']['visible_in'] : array();
		$class .= ' ' . SunFwSiteBootstrap::showOn($visible_in);
