...
class SunFwMenu
{
	...
	public static function prepareSettings($settings)
	{
		$rsettings = array(
		    ...
		    'mobileicon' => isset($settings['settings']['icon-mobile']) ? $settings['settings']['icon-mobile'] : 'fa-bars',
	...
	public static function render($menuType, $id, $settings)
	{
		...
		$html .= self::beginMenu($id, $rsettings['mobiletarget'], $rsettings['showsubmenu'], $rsettings['subeffect'], $rsettings['mobileicon']);
	 ...
	 * @return string
	 */
	public static function beginMenu($id, $mobile_target, $showsubmenu, $subeffect, $mobileicon)
	{
		...
		if ($mobile_target == 1)
		{
			return '<nav class="' . SunFwSiteBootstrap::getClass(
					'navbar navbar-default sunfw-menu-head navbar-expand-md navbar-expand-lg navbar-expand-xl'
				) . $classsubeffect . '" role="navigation">
				<div class="navbar-header">
					<button type="button" class="' . SunFwSiteBootstrap::getClass(
						'navbar-toggle collapsed btn btn-danger'
					) . '" data-toggle="collapse" id="menu_' . $id . '_mobile" data-target="#menu_' . $id . '" aria-expanded="false">
						<i aria-hidden="true" class="fa ' . ($mobileicon != '' ? $mobileicon : 'fa-bars') . '"></i>
					</button>
				</div>
				<div class="collapse navbar-collapse sunfw-menu" id="menu_' .
				$id . '"><ul class="nav navbar-nav sunfw-tpl-menu">';
		}
		else
		{
			return '<nav class="' . SunFwSiteBootstrap::getClass('navbar navbar-default navbar-expand sunfw-menu-head') .
				$classsubeffect . ' noMobileTagert" role="navigation">
				<div class="navbar-collapse sunfw-menu" id="menu_' .
				$id . '"><ul class="nav navbar-nav sunfw-tpl-menu">';
		}
	}
	...
	public static function renderComponentItemLayout($item, $activeID, $showicon = 0, $showdescription = 0, $megamenu = '')
	{
		...
		if (!empty($megamenu) || $item->deeper)
		{
			$class .= ' ' . SunFwSiteBootstrap::getClass('dropdown-toggle');
			$caret = '<span class="' . SunFwSiteBootstrap::getClass('caret') . '"></span>';
		}
	...
	public static function renderSeparatorItemLayout($item, $activeID, $showicon = 0, $showdescription = 0, $megamenu = '')
	{
		...
		if (!empty($megamenu) || $item->deeper)
		{
			$params = ' class="' . SunFwSiteBootstrap::getClass('dropdown-toggle') . '" data-toggle="dropdown" ';
			$caret = '<span class="' . SunFwSiteBootstrap::getClass('caret') . '"></span>';
		}
	...
	public static function renderMegaMenu($root)
	{
		...
		foreach ($root['rows'] as $rowIndex)
		{
			...
			foreach ($row['columns'] as $columnIndex)
			{
				...
				$classes = 'col-' . (SunFwSiteBootstrap::getVersion() === '4' ? 'md' : 'xs') . "-{$column['width']}"
					. (@isset($column['settings']['class']) ? " {$column['settings']['class']}" : '');

				$megaMenuHtml .= '
							<div id="' . $columnID . '" class="' . $classes . '">';
